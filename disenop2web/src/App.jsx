...
function App() {
  ...
  // The loaded history split into routes (see dividirEnRutas)
  ...
  // The route on screen as [lat, lon] pairs, the shape Leaflet expects
  const ruta = puntosRutaMostrada.map((punto) => [
    Number(punto.latitud),
    Number(punto.longitud),
  ]);

  // Same route for the sidebar list: newest point first
  ...
  // One route back. Pinning an index stops "live" mode from following the newest route.
  ...
  // Back to live mode: no explicit range, last 24 h, newest route
  const quitarFiltro = () => {
    setRangoActivo(null);
    setHistorialCargado(false);
    setIndiceRuta(null);
  ...
  // Page title: GPSLink plus the person's name from VITE_NOMBRE_PERSONA (build time)
  useEffect(() => {
