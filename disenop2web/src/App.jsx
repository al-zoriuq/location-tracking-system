...
function dividirEnRutas(historial) {
  ...
  for (let i = 1; i < historial.length; i++) {
    ...
    const velocidadKmh =
      diffHoras > 0 ? distanciaM / 1000 / diffHoras : Infinity;
    ...
    if (
      diffMs > UMBRAL_NUEVA_RUTA_MS ||
      distanciaM > UMBRAL_NUEVA_RUTA_METROS
    ) {
    ...
    const obtenerUbicacion = async () => {
      try {
        const response = await fetch(
          import.meta.env.BASE_URL + "api/ultima-ubicacion",
        );
        if (!response.ok) {
          setLocation(null);
          return;
        ...
        {location ? (
          <>
            <div className="panel">
              ...
              <div className="meta">
                <span>
                  {fechaGPS.toLocaleDateString("es-CO", OPCIONES_ZONA)}
                </span>
                <span>
                  {fechaGPS.toLocaleTimeString("es-CO", OPCIONES_ZONA)}
                </span>
              </div>
            ...
            {filtroAbierto ? (
              <div className="filtro-fecha">
                ...
                <div className="filtro-acciones">
                  ...
                  {rangoActivo && (
                    <button onClick={quitarFiltro}>Ver en vivo</button>
                  )}
            ...
            <div className="controles-mapa">
              ...
              >
                Carretera:{" "}
                {snapActivo ? (snapCargando ? "ajustando…" : "ON") : "OFF"}
              </button>
            </div>
            ...
            {ruta.length > 1 && (
              <div className="legend">
                ...
                <div className="legend-item">
                  <span
                    className={`legend-dot ${siguiendoActual ? "current" : "end"}`}
                  ></span>
